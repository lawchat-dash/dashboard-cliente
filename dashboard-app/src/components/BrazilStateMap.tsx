...
import { extractStateFromPhone, STATE_NAMES } from '@/utils/dddToState';
...
type ViewMode = 'leads' | 'contratos';

// SVG paths for Brazilian states (viewBox: 0 0 450 460)
const STATE_PATHS: Record<string, { d: string; labelX: number; labelY: number }> = {
  ...
  PB: {
    ...
    labelX: 434, labelY: 149,
  },
};

function getColorIntensity(value: number, maxValue: number): string {
  if (maxValue === 0 || value === 0) return 'hsl(var(--muted))';
  const ratio = Math.min(value / maxValue, 1);
  // From light blue to deep primary
  const lightness = 90 - ratio * 50; // 90% -> 40%
  const saturation = 40 + ratio * 40; // 40% -> 80%
  return `hsl(210, ${saturation}%, ${lightness}%)`;
}

const BrazilStateMap = ({ sessions, cards }: BrazilStateMapProps) => {
  const { classify } = useClassify();
  const [mode, setMode] = useState<ViewMode>('leads');
  const [hoveredState, setHoveredState] = useState<string | null>(null);
  const [showAll, setShowAll] = useState(false);

  // Build set of cardIds from filtered cards
  const filteredCardIds = useMemo(() => {
    return new Set(cards.filter(c => !c.archived).map(c => c.id));
  }, [cards]);

  // Map cardId -> phone from sessions (only for filtered cards)
  const cardPhoneMap = useMemo(() => {
    const map = new Map<string, string>();
    if (!sessions) return map;
    sessions.forEach(s => {
      if (s.cardId && s.contactPhone && filteredCardIds.has(s.cardId)) {
        map.set(s.cardId, s.contactPhone);
      }
    });
    return map;
  }, [sessions, filteredCardIds]);

  // Leads by state: count sessions whose cardId is in filteredCards
  const leadsByState = useMemo(() => {
    const counts = new Map<string, number>();
    if (!sessions) return counts;
    sessions.forEach(s => {
      if (!s.cardId || !filteredCardIds.has(s.cardId)) return;
      const state = extractStateFromPhone(s.contactPhone);
  ...
  // Contratos by state
  const contratosByState = useMemo(() => {
    const counts = new Map<string, number>();
    if (!cards) return counts;
    cards.forEach(c => {
      if (c.archived) return;
      if (classify(c) !== 'CONTRATO FECHADO') return;
      const phone = cardPhoneMap.get(c.id);
      if (!phone) return;
      const state = extractStateFromPhone(phone);
      if (state) counts.set(state, (counts.get(state) || 0) + 1);
    });
    return counts;
  ...
  const maxValue = useMemo(() => Math.max(...Array.from(activeCounts.values()), 1), [activeCounts]);

  const allStates = useMemo(() => {
    return Array.from(activeCounts.entries()).sort((a, b) => b[1] - a[1]);
  }, [activeCounts]);
  const topStates = useMemo(() => showAll ? allStates : allStates.slice(0, 5), [allStates, showAll]);

  return (
    <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-[0_1px_3px_rgba(15,23,42,0.05)]">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold text-foreground">Contatos por Estado</h3>
          <p className="text-[11px] text-muted-foreground">Distribuição geográfica dos seus contatos</p>
        </div>
        <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary">
          {total} {mode === 'leads' ? 'leads' : 'contratos'}
        </span>
      </div>
      ...
      <div className="relative mb-4">
        ...
        >
          {Object.entries(STATE_PATHS).map(([uf, { d }]) => {
            const count = activeCounts.get(uf) || 0;
            const fill = getColorIntensity(count, maxValue);
          ...
          {/* Labels for hovered state */}
          {hoveredState && STATE_PATHS[hoveredState] && (
            <text
              x={STATE_PATHS[hoveredState].labelX}
              y={STATE_PATHS[hoveredState].labelY}
              textAnchor="middle"
        ...
        {/* Tooltip */}
        {hoveredState && (
          <div className="pointer-events-none absolute left-1/2 top-2 z-10 -translate-x-1/2 rounded-lg border border-border bg-popover px-3 py-1.5 text-xs shadow-md">
            <span className="font-bold text-foreground">{hoveredState}</span>
            <span className="text-muted-foreground"> — {STATE_NAMES[hoveredState] || hoveredState}</span>
            <span className="ml-2 font-semibold text-primary">{activeCounts.get(hoveredState) || 0}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2.5">
        {topStates.map(([uf, count], i) => {
          const maxCount = topStates.length ? topStates[0][1] : 1;
          const pct = total > 0 ? (count / total) * 100 : 0;
          const barPct = maxCount > 0 ? (count / maxCount) * 100 : 0;
          return (
            <div key={uf} className="group flex items-center gap-2">
              <span className="w-7 text-xs font-bold text-foreground">{uf}</span>
              <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-muted">
                <motion.div
                  className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-green-500"
                  style={{ boxShadow: '0 0 8px rgba(21,191,65,0.3)' }}
                  initial={{ width: 0 }}
              ...
              <span className="w-10 text-right text-xs font-semibold tabular-nums text-foreground">
                <AnimatedCounter value={count} duration={900} />
              </span>
        ...
        {topStates.length === 0 && (
          <p className="py-4 text-center text-xs text-muted-foreground">Sem dados de telefone</p>
        )}
      </div>

      {/* Ver todos os estados */}
      {allStates && allStates.length > 5 && (
        <button
          onClick={() => setShowAll(v => !v)}
          className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl border border-border/50 bg-muted/30 py-2.5 text-sm font-medium text-muted-foreground transition-all hover:bg-muted/50 hover:text-foreground"
        >
          {showAll ? 'Ver menos' : `Ver todos os estados (${allStates.length})`}
